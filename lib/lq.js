...
angular.module('lq.core', [])
  ...
  .run(($rootScope, $location, QRC) => {
    ...
    function Control(name, component) {
      ...
      return new Proxy(self, {
        ...
        set: (_, prop, val) => QRC.SetValue(component, name, prop, val) || true
      })
    }
  ...
  .directive('lqToggle', () => {
    return {
      ...
      link: function(scope, el, attr) {
        const value = attr.lqToggle + '.Value';
  ...
  .directive('lqMomentary', () => {
    return {
      restrict: 'A',
      link: function(scope, el, attr) {
        const value = attr.lqMomentary + '.Value';
        scope.$watch(value, (value) => {
          el.toggleClass('active', value == 1);
        })
  ...
  .directive('lqString', () => {
    return {
      restrict: 'A',
      link: function(scope, el, attr) {
        const control = scope.$eval(attr.lqString);
        el.on('mouseup', () => { control.String = attr.value; });
        scope.$watch(attr.lqString + '.String', (value) =>
